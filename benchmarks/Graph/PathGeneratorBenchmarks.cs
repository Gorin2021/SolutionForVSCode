...
[MemoryDiagnoser]
public class PathGeneratorBenchmarks
{
    private GraphWithListOfWeight<int>? _smallGraph;
    private GraphWithListOfWeight<int>? _largeGraph;
    [GlobalSetup]
    public void Setup()
    {
        // Инициализация данных для тестов
        _smallGraph = CreateSmallGraphWithSpecialRoutes()[0];
        _largeGraph = CreateLargeGraphWithSpecialRoutes();
    }

    [Benchmark]
    public void GeneratePaths_SmallGraph()
    {
        var graph = _smallGraph!;
        var pathGenerator = new PathGenerator<int>(graph);
        pathGenerator.GeneratePaths(1, 10);
    }

    [Benchmark]
    public void GeneratePaths_LargeGraph()
    {
        var graph = _largeGraph!;
        var pathGenerator = new PathGenerator<int>(graph);
        pathGenerator.GeneratePaths(256, 56000);
    }
    private static GraphWithListOfWeight<int>[] CreateSmallGraphWithSpecialRoutes()
    {
        ...
        return new[] { graphWithListOfWeight };
    }
